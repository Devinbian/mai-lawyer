...
Page({
  ...
  calculateContentHeight() {
    ...
    query.select('.favorite-list').boundingClientRect((rect) => {
      if (rect) {
        this.setData({
  ...
  loadFavoriteList(isLoadMore = false) {
    ...
    setTimeout(() => {
      const mockData = Array(5).fill({}).map((_, index) => ({
        ...
        title: '示例文档示例文档示例文档示例文档示例文档示例文档示例文档示例文档示例文档示例文档示例文档' + (this.data.favoriteList.length + index + 1),
      ...
      this.setData({
        favoriteList: isLoadMore ? [...this.data.favoriteList, ...mockData] : mockData,
        isLoading: false,
